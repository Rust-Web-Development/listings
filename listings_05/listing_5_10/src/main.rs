...
async fn update_question(
    ...
    match store.questions.write().get_mut(&QuestionId(id)) {
        Some(q) => *q = question,
        None => return Err(warp::reject::custom(error::error::Error::QuestionNotFound)),
    }
...
fn extract_pagination(params: HashMap<String, String>) -> Result<Pagination, error::error::Error> {
    if params.contains_key("start") && params.contains_key("end") {
        return Ok(Pagination {
            start: params
                ...
                .unwrap()
                .parse::<usize>()
                .map_err(error::error::Error::ParseError)?,
            end: params
                .get("end")
                .unwrap()
                .parse::<usize>()
    ...
    Err(error::error::Error::MissingParameters)
}
...
async fn main() {
    ...
    let routes = get_questions
        ...
        .or(add_answer)
        .with(cors)
        .recover(error::error::return_error);
